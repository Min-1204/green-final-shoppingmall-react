export const validate = (signUpForm) => {
  // 유효성 검사 함수
  ...
  if (!signUpForm.loginId || signUpForm.loginId.length < 4) {
    ...
    e.loginId = "아이디는 영어(대/소문자)와 숫자만 사용할 수 있습니다.";
  }

  // 비밀번호 검사 로직
  if (!signUpForm.password) {
    e.password = "비밀번호를 입력하세요.";
  } else if (/\s/.test(signUpForm.password)) {
    e.password = "비밀번호에 공백은 사용할 수 없습니다.";
  } else if (!passwordRegex.test(signUpForm.password)) {
    e.password = "영문, 숫자, 특수문자를 포함하여 8자 이상 입력해주세요.";
  }
  ...
  // 연락처 검사 로직
  if (!signUpForm.phoneNumber || !/^\d{11}$/.test(signUpForm.phoneNumber))
    e.phoneNumber = "휴대전화 숫자만 11자리 입력하세요.";
